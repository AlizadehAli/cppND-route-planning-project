...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    float h = (*node).distance(*end_node); // node->distance(*end_node) can be used instead
    return h;
}
...
RouteModel::Node *RoutePlanner::NextNode() {
    // sort according to the f-value = (g-value + h-value)
    std::sort(open_list.begin(), open_list.end(), compareNodes);

    RouteModel::Node *lowest_f_value = open_list.back();
    open_list.pop_back();
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    while(currentNode != start_node) {
        ...
        distance += (*currentNode).distance(*currentNode->parent);
        currentNode = (*currentNode).parent;
    }
    path_found.push_back(*currentNode);

    // correct the order of the vector, first with start noe, and ending with end node.
